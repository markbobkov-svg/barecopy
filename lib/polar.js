...
const createCheckout = (productId, successUrl, embedOrigin) =>
  polar('/v1/checkouts/', {
    ...
    body: JSON.stringify({
      ...
      // No discount-code field in the checkout: the launch discount is done by
      // setting the price directly in Polar, so a promo-code box would only
      // confuse buyers. Flip to true if we ever run actual coupon codes.
